...
import { analyzePostureVisually, drawPostureVisualization, PostureAnalysis } from '../lib/postureVisualizer';
...
export default function Capture({ onComplete }: Props) {
  ...
  const [phase, setPhase] = useState<'loading' | 'ready' | 'capturing' | 'processing'>('loading');
      ...
      try {
        ...
        if (videoRef.current) {
          ...
          videoRef.current.addEventListener('loadedmetadata', () => {
            setPhase('ready');
            setStatus('Ready to start - Stand in frame');
          });
        }
      } catch (err) {
  ...
  const drawAnalysisFrame = (landmarks: Landmark[], videoWidth: number, videoHeight: number) => {
    if (!canvasRef.current) return;
    // Use visual posture analysis with green/red indicators
    const analysis = analyzePostureVisually(landmarks);
  ...
  const processContinuously = async () => {
    ...
    if (result && result.landmarks) {
      ...
      const postureAnalysis = analyzePostureVisually(result.landmarks);
